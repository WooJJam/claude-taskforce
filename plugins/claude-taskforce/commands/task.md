...
이 워크플로우는 `/task`로 명시적으로 호출됐을 때만 실행한다. 그 외 일반 대화 요청에는 발동하지 않는다.

단, `/task`로 호출됐더라도 **요청 규모가 워크플로우에 맞지 않으면**(예: 오타 수정, 한 줄 변경, 단순 질의처럼 설계·리뷰·QA가 과한 작업) 풀 사이클을 강제하지 않는다. 이 경우 그 사유를 사용자에게 알리고, 워크플로우 없이 일반 처리로 전환할지 확인한다.
