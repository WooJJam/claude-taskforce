# 역할별 AI 가상 개발팀 (claude-taskforce)
...
`claude-taskforce`은 사람 개발팀의 워크플로우(요구사항 분석 · 설계 · TDD · 코드 리뷰 · QA)를 **역할별 AI 에이전트**로 분리한 Claude Code 플러그인입니다. 6개의 독립 에이전트를 Orchestrator가 순차 연결하며, 개발자는 설계 단계에서 한 번만 승인하면 나머지는 자동 진행됩니다.
...
/plugin install claude-taskforce
```
...
```
plugins/claude-taskforce/skills/implementer/references/node.md
plugins/claude-taskforce/skills/qa/references/node.md
```
